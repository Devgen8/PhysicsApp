...
class TestDescriptionTableViewCell: UITableViewCell {
    ...
    func setCellsElements(with ratio: CGFloat) {
        contentView.addSubview(descriptionImage)
        descriptionImage.translatesAutoresizingMaskIntoConstraints = false
        descriptionImage.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 25).isActive = true
        descriptionImage.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -25).isActive = true
        descriptionImage.topAnchor.constraint(equalTo: taskNameLabel.bottomAnchor, constant: 5).isActive = true
        descriptionImage.heightAnchor.constraint(equalToConstant: (UIScreen.main.bounds.width - 50) * ratio).isActive = true
    ...
    @objc func imageTapped() {
        imageOpener?.openImage(descriptionImage.image ?? UIImage())
    }
